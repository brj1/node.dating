...
BEAST performed the best of all methods when running $10^8$ steps, but this also took the most time.
With $10^4$ steps BEAST performed terribly and in around the same amount of time as \code{node.dating} at 100 steps.
However, the results using BEAST are not completely comparable since we used the sequence data for BEAST instead of the reconstructed phylogenies.
...
\begin{table}[t]
	...
	\begin{tabular}{lcc}
		...
		\hline
		\code{node.dating} (initial) & 1.63 & 29.2 \\
		\code{node.dating} (1 step) & 2.26 & 26.6 \\
		\code{node.dating} (10 steps) & 7.72 & 23.7 \\
...
However, the local likelihood model can be extended to incoporate a variable moelcular clock; future work would like to include this extension.
...
% remind latex that this section fits here
\\\\\\
